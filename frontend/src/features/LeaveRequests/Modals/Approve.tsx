...
const ApproveModal: React.FC<ApproveModalProps> = ({ 
  ...
  const prediction = useMemo(() => {
    ...
    // Check special leave
    const isSpecialLeave = (SPECIAL_LEAVES_NO_DEDUCTION as readonly string[]).includes(request.leaveType || request.leave_type);
    if (isSpecialLeave) {
    ...
    const getBalance = (type: string | null) => {
      if (!type) return 0;
      const credit = credits.find((c: LeaveBalance) => c.credit_type === type);
