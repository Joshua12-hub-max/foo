// ReviewModal.jsx for Undertime (in ../../components/Modal UI/UndertimeFormAdminModal/Review)
import { useState } from "react";
import { X, FileText, Send, CheckCircle, XCircle, Clock, Download, User, Calendar, FileType, MessageSquare, AlertCircle } from "lucide-react";

export default function ReviewModal({ isOpen, undertimeForms, handleApprove, handleReject, selectedRequests, onClose }) {
  ...
  const pendingRequests = undertimeForms?.filter(
    (form) => form.reviewRequest === "Pending" && selectedRequests.includes(form.id)
  ) || [];
  ...
  const confirmApprove = () => {
    if (selectedRequest && handleApprove) {
  ...
  const confirmReject = () => {
    ...
    if (selectedRequest && handleReject) {
      setProcessingId(selectedRequest.id);
        ...
        >
          ...
          <div className="bg-gradient-to-r from-slate-950 to-green-700 flex items-center justify-between p-4 border-b">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-white">Review Undertime Requests</h2>
            </div>
            <button 
              onClick={onClose} 
          ...
          <div className="p-4 overflow-y-auto flex-1">
            {pendingRequests.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm mt-1">All undertime requests have been reviewed</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200">
                      ...
                      <span className="text-xs text-gray-500">
                        Submitted: {formatDate(form.submittedDate || new Date())}
                      </span>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        {form.formFile ? (
                          <a
                            href={form.formFile}
                            download={form.fileName || "Undertime_Request_Form.pdf"}
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-blue-700 font-medium">{form.fileName || "Undertime Request Form.pdf"}</span>
                            </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl max-w-sm w-full p-6 animate-in fade-in zoom-in duration-200">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                Approve Undertime Request?
              </h3>
              <p className="text-sm text-gray-600 mb-4">
                You are about to approve <span className="font-semibold">{selectedRequest.employeeName}'s</span> undertime request for <span className="font-semibold">{selectedRequest.date}</span>.
              </p>
              <div className="bg-gray-50 rounded-lg p-3 mb-4 text-left">
                <p className="text-xs text-gray-600">
                  <span className="font-semibold">Time Out:</span> {selectedRequest.timeOut}
                </p>
                <p className="text-xs text-gray-600">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-lg shadow-2xl max-w-sm w-full p-6 animate-in fade-in zoom-in duration-200">
            <div className="text-center mb-4">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                Reject Undertime Request
              </h3>
              <p className="text-sm text-gray-600">
                Please provide a reason for rejecting <span className="font-semibold">{selectedRequest.employeeName}'s</span> request.
              </p>
            </div>
            <div className="mb-4">
              <textarea
