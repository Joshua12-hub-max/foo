...
const ApplicantCard = memo(({ applicant, isDragging, onViewRequirements }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  // Hardware-accelerated transform with smooth transition
  const style = useMemo(() => ({
  ...
  // Skip rendering full card if being dragged (DragOverlay shows instead)
