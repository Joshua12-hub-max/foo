import { ExternalLink, SquarePen, Trash2, Share2 } from 'lucide-react';
import TableSkeleton from '@components/Custom/Shared/TableSkeleton';

const JobTable = ({ loading, filteredJobs, onEdit, onDelete, onShare, onView }) => {
            ...
            {filteredJobs.length > 0 ? filteredJobs.map(job => (
                ...
                <td className="px-6 py-4 text-sm whitespace-nowrap">
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        {job.fb_post_id && (
                        <a 
                            href={`https://facebook.com/${job.fb_post_id}`} 
                            target="_blank" 
                        ...
                        >
                            ...
                            Facebook <ExternalLink size={9} />
                        </a>
                        )}
                        {job.linkedin_post_id && (
                        <a 
                            href={`https://www.linkedin.com/feed/update/${job.linkedin_post_id}`} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            onClick={e => e.stopPropagation()}
                            className="text-[10px] text-[#0A66C2] hover:underline flex items-center gap-1"
                        >
