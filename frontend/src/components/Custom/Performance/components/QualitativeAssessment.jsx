...
const QualitativeAssessment = ({
  ...
  const handleSaveModal = (formData) => {
     ...
     if (editingAssessment?.id) { if (onEdit) onEdit({ ...editingAssessment, ...savedData }); }
     else { if (onAdd) onAdd(savedData); }
     
     setShowEditModal(false);
     setEditingAssessment(null);
  };

  const handleDeleteClick = (item) => { setDeletingAssessment(item); setShowDeleteModal(true); };
            ...
            <div className="px-4 py-3 flex justify-between items-start border-b border-gray-50 bg-white">
               <div className="flex items-center gap-2">
                  <div className={`p-1.5 rounded bg-gray-50 border border-gray-100 text-gray-400 group-hover:text-gray-900 transition-all`}>
                      {ICON_MAP[assessment.iconName] || ICON_MAP['MessageSquare']}
                  </div>
                  <div>
