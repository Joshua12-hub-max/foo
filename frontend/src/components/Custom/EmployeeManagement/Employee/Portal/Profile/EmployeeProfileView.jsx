...
const DataField = ({ label, value, icon: Icon, fullWidth = false, highlight = false }) => (
...
// Section Container
const Section = ({ title, children, icon: Icon, columns = "grid-cols-2 md:grid-cols-4 lg:grid-cols-6" }) => (
  <div className="mb-6">
...
const MasterProfileView = ({ profile, loading, error, onRefresh, isAdmin = false, onStatusChange }) => {
    ...
    <div className="w-full bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mb-12">
      ...
      <div className="p-6">
        ...
        <Section title="Employment Record" icon={Briefcase}>
          ...
          <DataField label="Position Title" value={profile.position_title || profile.positionTitle || profile.job_title} highlight />
