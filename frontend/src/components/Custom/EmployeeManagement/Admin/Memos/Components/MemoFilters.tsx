...
const MemoFilters: React.FC<MemoFiltersProps> = memo(({ filters, onFilterChange, onSearch, onClear }: MemoFiltersProps) => {
    ...
    <div className="bg-[#F8F9FA] p-4 rounded-lg shadow-md mb-6">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
        ...
        <div className="md:col-span-1 z-20 relative">
          <Combobox
            options={[{ value: '', label: 'All Types' }, ...MEMO_TYPES]}
            value={filters.memoType}
            onChange={(val) => onFilterChange('memoType', val)}
            placeholder="All Types"
            buttonClassName="w-full bg-white border border-gray-200 rounded-lg shadow-sm px-3 py-2 text-sm"
          />
        </div>

        {/* Status Filter */}
        <div className="md:col-span-1 z-10 relative">
          <Combobox
            options={[{ value: '', label: 'All Status' }, ...MEMO_STATUSES]}
