...
export const Filters = ({filters, onFilterChange, onApplyFilters, onClear, onNewRequest, isLoading}) => {
  // Renaming onFilterChange to handleChange as per the provided code snippet
  const handleChange = onFilterChange;
  // Renaming onApplyFilters to onApply as per the provided code snippet
  const onApply = onApplyFilters;
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
        ...
        <div className="md:col-span-2 flex gap-2 w-full">
          <button
            onClick={onApply}
            disabled={isLoading}
            className="flex-1 flex items-center justify-center gap-2 bg-[#F8F9FA] text-gray-700 font-medium px-4 py-2 rounded-lg text-sm shadow-sm hover:bg-[#F8F9FA] transition-all disabled:opacity-50 active:scale-95 whitespace-nowrap border border-gray-200"
          >
            Apply
          </button>
          <button
