...
const DailyTimeRecord = () => {
  ...
  const uniqueDepartments = useMemo(() => {
    return [...new Set(dtrData.map(item => item.department))].sort();
  }, [dtrData]);

  const uniqueEmployees = useMemo(() => {
    return [...new Set(dtrData.map(item => item.name))].sort();
  }, [dtrData]);
  ...
  const handleExportCSV = useCallback(async () => {
    ...
    try {
      ...
      const headers = ['Status', 'Employee ID', 'Employee Name', 'Department', 'Date', 'Time In', 'Time Out', 'Hours Worked', 'Remarks'];
      const csvContent = [
        headers.join(','),
      ...
      link.setAttribute('download', `dtr_${today.replace(/\//g, '-')}.csv`);
    ...
    <div className={`min-h-screen flex flex-col bg-gradient-to-br from-neutral-100 to-stone-100 rounded-xl shadow-xl p-7 w-full overflow-hidden text-gray-800 transition-all duration-300 ${sidebarOpen ? 'max-w-[1400px] xl:max-w-[77vw]' : 'max-w-[1600px] xl:max-w-[88vw]'}`}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-800 mt-1">Employee time record management</p>
        </div>
        ...
        <div className="overflow-x-auto bg-gray-50 rounded-lg">
          <table className="w-full min-w-[1200px]">
            <thead className="bg-[#274b46] text-[#F8F9FA]">
              <tr>
                {["Status", "Employee ID", "Employee Name", "Department", "Date", "Time In", "Time Out", "Hours Worked", "Remarks"].map((header) => (
