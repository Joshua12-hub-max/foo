...
import { useForm, FieldErrors, Control, Resolver } from "react-hook-form";
...
import {
  usePublicJobDetail,
  useJobApplication,
} from "@/features/Recruitment/hooks/usePublicJobs";
...
const JobDetail = () => {
  ...
  const {
    ...
    watch,
    trigger,
    formState: { errors, isLoading: isFormLoadingState },
  ...
  const mutation = useJobApplication(
    (response) => {
      ...
      const data = response.data as JobApplicationResponse;
    ...
    (err: AxiosError<RecruitmentErrorResponse>) => {
      ...
      if (errorData?.errors && typeof errorData.errors === "object") {
        ...
        showToast(
          errorData.hint ||
            `${errorCount} field(s) need attention. Please review the form.`,
          "error",
        );
        ...
        showToast(
          errorData?.message ||
            "Invalid application data. Please review your entries.",
          "error",
        );
      } else {
        showToast(
  ...
  const onSubmit = async (data: JobApplicationSchema) => {
    ...
    console.log("Job duty type:", job?.dutyType);

    mutation.mutate({ id, data });
  };

  const onFormError = (formErrors: FieldErrors<JobApplicationSchema>) => {
    console.error("=== FORM VALIDATION ERRORS ===", formErrors);
    const errorMessages = Object.entries(formErrors).map(([field, error]) => {
      const message = error && typeof error === 'object' && 'message' in error ? String(error.message) : "Invalid value";
      return `${field}: ${message}`;
    });
    setValidationErrors(errorMessages);
    showToast(`${errorMessages.length} field(s) need your attention.`, "error");
    scrollToForm();
  };
    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center bg-slate-50 p-6 text-center font-sans">
        ...
        <p className="text-slate-500 mb-8 font-semibold text-sm">
          This job record has been archived or is no longer accepting
          submissions.
        </p>
        <button
          onClick={() => navigate("/careers/jobs")}
          className="px-6 py-3 bg-accent text-white rounded-xl font-bold text-xs tracking-widest hover:bg-accent-hover transition-all active:scale-95 shadow-sm uppercase"
        >
          Back to Listings
        </button>
      </div>
  ...
  interface StepProps {
    register: ReturnType<typeof useForm<JobApplicationSchema>>["register"];
    errors: FieldErrors<JobApplicationSchema>;
    setValue: ReturnType<typeof useForm<JobApplicationSchema>>["setValue"];
    watch: ReturnType<typeof useForm<JobApplicationSchema>>["watch"];
  ...
  interface Step {
    id: number;
    title: string;
    component: React.ElementType; // Better than any
  }
  ...
  // Renumber IDs
  const allSteps = baseSteps.map((step, idx) => ({
    ...step,
    id: idx + 1,
  }));

  const getStepIndex = (component: React.ElementType) => {
    return allSteps.findIndex((s) => s.component === component) + 1;
  };

  // 100% PRECISION: Define fields for each step for tactical validation
  const getStepFields = (stepId: number): (keyof JobApplicationSchema)[] => {
    switch (stepId) {
      case 1:
        return ["firstName", "lastName", "birthDate", "birthPlace", "sex", "civilStatus", "nationality"];
      case 2:
        return ["email", "phoneNumber", "emergencyContact", "emergencyContactNumber"];
      case 3:
        return ["resCity", "resBarangay", "resProvince", "zipCode"];
      case 4:
        if (job?.dutyType === "Standard") return ["gsisNumber", "pagibigNumber", "philhealthNumber", "tinNumber"];
        return [];
      case 5:
        if (job?.dutyType === "Standard") return ["education"];
        return [];
      case 6:
        if (job?.dutyType === "Standard") return ["workExperiences"];
        return [];
      case 7:
        if (job?.dutyType === "Standard") return ["eligibilities"];
        return [];
      default:
        return [];
    }
  ...
  const handleNext = async () => {
    ...
    if (fieldsToValidate.length > 0) {
      const isValid = await trigger(fieldsToValidate as (keyof JobApplicationSchema)[]);
      if (!isValid) {
        showToast("Please fill in all required fields correctly to continue.", "error");
        return;
      }
    }

    setCurrentStep((prev) => Math.min(allSteps.length, prev + 1));
    scrollToForm();
  };
              ...
              <div className="space-y-16">
                ...
                >
                  ...
                  job.otherQualifications ? (
                    <div className="space-y-4">
                      {job.education && (
                        <div className="p-6 bg-white border border-slate-200 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.06)] hover:border-slate-300 transition-all duration-300 group">
                          <div className="text-[10px] font-black text-slate-400 group-hover:text-slate-900 uppercase tracking-[0.2em] mb-2 transition-colors">
                            Education
                          </div>
                      ...
                      {job.otherQualifications && (
                        <div className="p-6 bg-white border border-slate-200 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.06)] hover:border-slate-300 transition-all duration-300 group">
                          ...
                          <div className="text-sm md:text-base font-bold text-slate-800 leading-relaxed whitespace-pre-wrap">
                            {job.otherQualifications}
                          </div>
                        </div>
                      )}
                    ...
                    <div className="text-sm md:text-[15px] text-slate-600 font-semibold leading-loose whitespace-pre-wrap">
                      {job.requirements ||
                        "Standard LGU hiring qualifications apply. No special qualifications declared."}
                    </div>
                  )}
                </motion.section>
              </div>
                ...
                {job.attachmentPath && (
                  <div className="bg-white border border-slate-200 shadow-xl shadow-slate-200/40 rounded-[2rem] p-8 space-y-6 relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      ...
                      <div>
                        <h4 className="text-[13px] font-black text-slate-900 uppercase tracking-widest mb-1.5">
                          Reference Docs
                        </h4>
                        <p className="text-[11px] text-slate-500 font-bold leading-relaxed">
                          Official qualification matrix for this position.
                        </p>
                      </div>
                    </div>
                    <a
              ...
              {success ? (
                ...
                >
                  ...
                  <div className="space-y-4 relative z-10">
                    ...
                    <p className="text-slate-600 font-semibold text-base max-w-sm mx-auto">
                      Your application package has been securely queued for HR
                      verification.
                    </p>
                  </div>
                  <button
                ...
                >
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-[10px] font-black text-slate-700 border border-slate-200 bg-slate-100 px-4 py-1.5 rounded-full inline-block uppercase tracking-[0.4em] shadow-sm">
                  ...
                  <form
                    ...
                    noValidate
                    className="space-y-6 bg-white border border-gray-100 shadow-xl p-6 md:p-8 rounded-2xl relative overflow-hidden"
                  >
                          ...
                          <div className="flex-1">
                            ...
                            <ul className="space-y-2">
                              {validationErrors.map((error, idx) => (
                                <li
                                  key={idx}
                                  className="text-xs font-semibold text-red-700 flex items-start gap-2"
                                >
                        ...
                        {allSteps.map((step, idx) => (
                          <div key={step.id} className="flex items-center min-w-fit">
                            <div className="flex flex-col items-center">
                              <div
                                className={` w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                                  currentStep === step.id
                                    ? "bg-accent text-white ring-4 ring-slate-200"
                                    : currentStep > step.id
                              ...
                              <span
                                className={`text-[10px] font-bold uppercase tracking-wider mt-2 text-center max-w-[80px] ${
                            ...
                            {idx < allSteps.length - 1 && (
                              <div
                                className={`w-8 md:w-16 h-1 mx-2 transition-all ${
                    ...
                    <div className="relative z-10 min-h-[400px]">
                      {React.createElement(allSteps[currentStep - 1].component as React.ComponentType<StepProps>, ({
                        register,
                        errors,
                        setValue,
                        watch,
