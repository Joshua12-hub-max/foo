    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 font-semibold mb-6">
          {" "}
          Provide your educational history (at least one level required for
          Standard duty jobs){" "}
        </p>{" "}
      </div>{" "}
      {/* Tab Navigation */}{" "}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          ...
          {educationLevels.map((level) => (
            <button
              ...
              className={`px-4 py-2 text-xs font-bold tracking-wider rounded-t-lg transition-all whitespace-nowrap ${activeTab === level ? "bg-slate-500 text-white border-b-2 border-slate-600" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
            >
              {" "}
              {level}{" "}
            </button>
      ...
      {/* Tab Content */}{" "}
      <div className="pt-4"> {renderEducationFields(activeTab)} </div>{" "}
      {errors.education && (
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <p className="text-xs text-blue-700 font-semibold">
          {" "}
          Note: For Standard duty jobs, at least one educational level must have
          a school name filled in.{" "}
        </p>{" "}
      </div>{" "}
    </div>
  );
