...
import { verifyFileHeader } from "@/utils/fileValidation";
...
const FileUploadSection: React.FC<FileUploadSectionProps> = ({
  ...
  const handleResumeChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isValidHeader) {
      alert(
        "File integrity check failed. The file content does not match its extension.",
      );
      e.target.value = "";
      setResumeFile(null);
      setValue("resume", null as unknown as File);
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log(
      "[FileUpload] Photo selected:",
      file.name,
      file.size,
      file.type,
    );
    setPhotoFile({ name: file.name, size: file.size, type: file.type });
    setValue("photo", file as unknown as File, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
  const handleCertChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (error) {
      ...
      e.target.value = "";
      setCertFile(null);
      setValue("eligibilityCert", null as unknown as File);
      return;
    }
    ...
    console.log(
      "[FileUpload] Certificate selected:",
      file.name,
      file.size,
      file.type,
    );
    setCertFile({ name: file.name, size: file.size, type: file.type });
    setValue("eligibilityCert", file as unknown as File, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
  const removeFile = (type: "resume" | "photo" | "cert") => {
    ...
    if (type === "resume") {
      ...
      setPhotoFile(null);
      setValue("photo", null as unknown as File, { shouldValidate: true });
    } else {
      setCertFile(null);
      setValue("eligibilityCert", null as unknown as File, {
        shouldValidate: true,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <h3 className="text-lg font-black text-slate-900 mb-1 tracking-widest flex items-center gap-3">
          {" "}
          <span className="w-1.5 h-1.5 bg-slate-500 rounded-full"></span> File
          Uploads{" "}
        </h3>{" "}
      ...
      <div className="space-y-3">
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-slate-500 transition-all">
          {" "}
          {!resumeFile ? (
            <div>
              {" "}
      ...
      {dutyType === "Standard" && (
        <div className="space-y-3">
          ...
          {errors.eligibilityCert && (
            <p className="text-red-500 text-xs mt-1 ml-1">
              {errors.eligibilityCert.message as string}
            </p>
          )}{" "}
        </div>
      )}{" "}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <p className="text-xs text-blue-700 font-semibold">
          {" "}
          File Requirements: Maximum file size is 10MB. Accepted formats are
          listed for each upload type.{" "}
        </p>{" "}
      </div>{" "}
    </div>
  );
