...
const ReviewSection: React.FC<ReviewSectionProps> = ({
  ...
  const handleEdit = (component: React.ElementType) => {
    if (getStepIndex) {
      const step = getStepIndex(component);
      setCurrentStep(step);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <h3 className="text-lg font-black text-slate-900 mb-1 tracking-widest flex items-center gap-3">
          {" "}
          <span className="w-1.5 h-1.5 bg-slate-500 rounded-full"></span> Review
          Application{" "}
        </h3>{" "}
      ...
      <InfoCard title="Personal Information" component={PersonalInfoSection}>
        {" "}
        <Field
          label="Full Name"
        ...
        <Field
          label="Height"
          value={formData.height ? `${formData.height} cm` : undefined}
        />{" "}
        <Field
          label="Weight"
          value={formData.weight ? `${formData.weight} kg` : undefined}
        />{" "}
      </InfoCard>{" "}
      {/* Contact Information */}{" "}
      <InfoCard title="Contact Information" component={ContactSection}>
        {" "}
      ...
      {/* Address Information */}{" "}
      <InfoCard title="Address Information" component={AddressSection}>
        {" "}
      ...
      {/* Education */}{" "}
      <InfoCard title="Educational Background" component={EducationSection}>
        {" "}
        {formData.education &&
            ...
            if (data?.school) {
              return (
                <div key={level} className="md:col-span-2">
                  ...
                  <p className="text-sm text-gray-800 font-medium mt-0.5">
                    {" "}
                    {data.school} {data.course && ` - ${data.course}`}{" "}
        ...
        {!formData.education ||
          (!Object.values(formData.education).some((d) => d?.school) && (
      ...
      {/* Work Experience */}{" "}
      <InfoCard title="Work Experience" component={ExperienceSection}>
        {" "}
        ...
        {formData.totalExperienceYears !== undefined && (
          <div className="md:col-span-2 pt-3 mt-3 border-t border-slate-100 flex items-center justify-between bg-slate-50 p-3 rounded-lg">
            <p className="text-xs font-black text-slate-500 uppercase tracking-widest">
              Total Relevant Experience
            </p>
            <p className="text-sm font-black text-slate-900 bg-white px-3 py-1 rounded-md border border-slate-200 shadow-sm">
              {formData.totalExperienceYears} Year(s)
            </p>
          </div>
        )}
      </InfoCard>{" "}
        ...
        {formData.eligibilities && formData.eligibilities.length > 0 ? (
          formData.eligibilities.map((elig, idx) => (
            <div key={idx} className="md:col-span-2">
              ...
              <p className="text-sm text-gray-800 font-medium mt-0.5">
                {" "}
                {elig.name} {elig.rating && ` - Rating: ${elig.rating}`}{" "}
          ...
          <p className="text-sm text-gray-500 md:col-span-2">
            No eligibility records provided
          </p>
        )}{" "}
      </InfoCard>{" "}
      {/* Training */}{" "}
      {formData.trainings && formData.trainings.length > 0 && (
        <InfoCard title="Training Programs" component={TrainingSection}>
          {" "}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            type="button"
            onClick={() => handleEdit(FileUploadSection)}
            className="text-xs font-bold text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-3 py-1 rounded transition-all"
          >
            {" "}
            Edit{" "}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            {" "}
            <span className="text-xs font-semibold text-gray-500 tracking-wider w-32">
              Resume:
            </span>{" "}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-800 font-medium">
              {" "}
              {formData.eligibilityCert instanceof File
                ? formData.eligibilityCert.name
                : "Not uploaded"}{" "}
            </span>{" "}
          </div>{" "}
        </div>{" "}
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <div className="text-sm">
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-700">
              ...
              <li>
                I understand that any false information may result in
                disqualification or termination
              </li>{" "}
              <li>I authorize the verification of the information provided</li>{" "}
              <li>
                I agree to comply with all terms and conditions of the
                application process
              </li>{" "}
            </ul>{" "}
          </div>{" "}
      ...
      {!termsAccepted && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          ...
          <p className="text-xs text-yellow-700 font-semibold">
            {" "}
            Please accept the terms and conditions to submit your
            application.{" "}
          </p>{" "}
        </div>
      )}{" "}
