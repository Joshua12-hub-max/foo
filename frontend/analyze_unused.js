...
const entryPoints = [
  ...
  path.join(srcDir, 'index.css'), // Usually imported by main, but good to be safe
];
...
function resolveImport(sourceFile, importPath) {
  ...
  if (importPath.startsWith('.')) {
    targetPath = path.resolve(path.dirname(sourceFile), importPath);
     ...
     // absolute import from root - uncommon in vite usually without alias, but let's assume src root
     targetPath = path.join(srcDir, importPath);
  } else {
    // node_modules or alias
    // Simple alias check for 'src/' or '@/' if common
...
while (queue.length > 0) {
  ...
  // Skip non-text files for parsing (like images, though images usually imported)
  // For simplicity, we parse js/jsx/css/json
  ...
  try {
    ...
    while ((match = importRegex.exec(content)) !== null) {
      ...
      // Handle CSS imports in JS
      // Handle static assets (images) - we will resolve them and mark used
