...
CREATE TABLE `plantilla_positions` (
	...
	`is_coterminous` boolean DEFAULT false,
...
CREATE TABLE `chat_messages` (
	...
	`sender_type` enum('Applicant','Administrator') NOT NULL,
	`sender_id` int,
...
CREATE TABLE `recruitment_applicants` (
	...
	`skills` text,
	`hired_date` datetime,
	`is_meycauayan_resident` boolean DEFAULT false,
...
CREATE TABLE `recruitment_jobs` (
	...
	`updated_at` timestamp DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	`attachment_path` varchar(255),
	`require_civil_service` boolean DEFAULT false,
	`require_government_ids` boolean DEFAULT false,
	`require_education_experience` boolean DEFAULT false,
...
CREATE TABLE `performance_reviews` (
	...
	`reviewer_rating_score` decimal(3,2),
